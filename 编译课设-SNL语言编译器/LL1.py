...
class GrammaticalAnalysis_LL1:
    ...
    def analysis(self,code_list,wordlist,linenum_list,root):
        ...
        top='输入流当前单词'+'                                                         '+"分析栈"
        window.f2.insert_n(top)

        tempChildren = []
        for i in ana_list:
            tempChildren.append(i.data)
        window.f2.insertn()
        window.f2.insert_n('                                                                      '+str(tempChildren))

        matchTags=self.matchTags

        while True:
            if ana_list == [] and pdeal==len(code_list)-2 and code_list[pdeal]=='.' and code_list[pdeal+1]=='EOF':
                window.f2.insertn("无语法错误")
                break
            else:
                if pdeal>=len(code_list):
                    window.f2.word='错误'
                    window.f2.error = '请检查输入，输入不能为空'
                    window.f2.display_messagebox()
                    self.flag=1
                    break
                # 分析栈顶为终极符且与输入流当前值相同
                    ...
                    window.f2.insert_d("MATCH:"+str(code_list[pdeal-1]),str(tempChildren))
                # 分析栈顶为非终极符，查表用相应产生式右部进行替换
                    ...
                    tempChildren=[]
                    for i in ana_list:tempChildren.append(i.data)
                    window.f2.insert_d(str(code_list[pdeal]),str(tempChildren))
                #出现语法错误
                else:
                    self.flag=1
                    window.f2.insert_n("\33[31mLL(1)分析中断，第" + str(linenum_list[pdeal]) + "行单词" + str(wordlist[pdeal]) + "出现语法错误")
...
def startLL1(obj_tokens):
    ...
    window.f2.insert_n("代码段中每个单词对应的终极符序列如下：")

    window.f2.insert_n(codelist)
    ...
    if (obj_LL1.flag == 1):
        window.f2.insert_n("\33[30m请按提示修改语法错误后，再进行后续工作")
